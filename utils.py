...
def load_random_env(env_folder):
    ...
    Returns:
        gym-environment, info
    """
    env_list0 = [os.path.join(env_folder, env_file) for env_file in os.listdir(env_folder)]
    env_list = [x for x in env_list0 if x.endswith('.env')]
...
def load_random_env_sequential(env_folder, id):
    """
    Load a random DoorKey environment
    ---------------------------------------------
    Returns:
